...
export default function NewOrderPage() {
  const [mercado, setMercado] = useState('COLOMBIA');
  const [observaciones, setObservaciones] = useState('');
  const navigate = useNavigate();

  const mutation = useMutation({
    mutationFn: () => orderApi.create({ mercado, observaciones: observaciones || undefined }),
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Mercado destino</label>
          <select value={mercado} onChange={(e) => setMercado(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none">
